  ...
  try {
    await api('/api/profiles/me', {
      ...
      body: {
        ...
        budgetMin: data.budgetMin ? Number(data.budgetMin) : 2000,
        budgetMax: data.budgetMax ? Number(data.budgetMax) : 9000,
        city: data.city && data.city.trim() ? data.city.trim() : 'Kumasi',
        bio: data.bio ?? '',
        smoker,
        smokerOk: smoker,
        hasPets: false,
        petsOk: data.lifestyle?.petFriendly !== 'No',
        seekingType: data.lifestyle?.seekingType ? mapSeekingType(data.lifestyle.seekingType) : 'SEEKING_ROOM',
